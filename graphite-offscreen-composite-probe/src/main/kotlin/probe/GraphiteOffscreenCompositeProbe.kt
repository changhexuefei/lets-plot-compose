...
fun main() {
    val outputDir = File(
        System.getenv("GRAPHITE_PROBE_OUTPUT_DIR")
            ?: "graphite-offscreen-composite-probe/build/probe"
    ).apply { mkdirs() }
    val resultFile = outputDir.resolve("graphite-offscreen-composite.txt")
    val softwarePng = outputDir.resolve("software-direct.png")
    val graphitePng = outputDir.resolve("graphite-offscreen.png")
    val compositePng = outputDir.resolve("graphite-offscreen-composited.png")

    var stage = "startup"
    var prepared: PreparedPlot? = null
    val evidence = linkedMapOf(
        "schema" to "1",
        "probe.type" to "graphite-offscreen-composite",
        "renderer.entry" to "PlotCanvasDrawable -> paintOnSkiaCanvas -> Graphite offscreen -> CPU readback -> Skia Image -> raster Canvas",
        "bridge.source" to "DesktopSkiaPaintBridge.kt",
        "bridge.density" to BRIDGE_DENSITY.toString(),
        "bridge.plot_position" to "$PLOT_X,$PLOT_Y",
        ...
        "plotpanel.integration" to "OFFSCREEN_COMPOSITE_PROBE",
        "compose.native_canvas.integration" to "SIMULATED_RASTER_CANVAS",
        "offscreen.transfer" to "CPU_READBACK",
        "offscreen.composite" to "REQUESTED",
    ...
    try {
        check(System.getProperty("os.name").startsWith("Windows", ignoreCase = true)) {
            "Graphite offscreen composite probe is intentionally Windows-only"
        }
        ...
        evidence["software.paint_bridge"] = "PASS"
                ...
                context.makeRecorder().use { recorder ->
                    BackendTexture.makeVulkan(
                        ...
                        surface.use {
                            ...
                            evidence["graphite.paint_bridge"] = "PASS"
                            ...
                            stage = "graphite-snapshot"
                            surface.makeImageSnapshot().use { snapshot ->
                                check(snapshot.width == WIDTH && snapshot.height == HEIGHT) {
                                    "Unexpected Graphite snapshot size: ${snapshot.width}x${snapshot.height}"
                                }
                                evidence["graphite.snapshot"] = "PASS"
                                evidence["graphite.snapshot.size"] = "${snapshot.width}x${snapshot.height}"
                            }
        ...
        stage = "software-composite"
        val compositePixels = compositeReadbackOnSoftwareCanvas(graphitePixels, compositePng)
        evidence.putAll(assertPlotStructure("composite", compositePixels))
        evidence["software.composite"] = "PASS"
        evidence["offscreen.composite"] = "PASS"

        stage = "backend-comparison"
        val difference = pixelDifferenceRatio(softwarePixels, compositePixels)
        ...
        check(difference <= MAX_PIXEL_DIFFERENCE_RATIO) {
            "Graphite offscreen composite output diverged from SOFTWARE baseline: " +
                "difference=$difference max=$MAX_PIXEL_DIFFERENCE_RATIO"
        }
        evidence["comparison"] = "PASS"

        evidence["result"] = "OFFSCREEN_COMPOSITE_CAPABLE"
        evidence["failure.stage"] = "none"
        writeEvidence(resultFile, evidence)
        println("GRAPHITE_OFFSCREEN_COMPOSITE_RESULT PASS")
        ...
        writeEvidence(resultFile, evidence)
        println("GRAPHITE_OFFSCREEN_COMPOSITE_RESULT FAIL stage=$stage type=" + evidence["failure.type"])
        throw t
    } finally {
...
private fun createVulkanObjects(): VulkanObjects {
    MemoryStack.stackPush().use { stack ->
        val appInfo = VkApplicationInfo.calloc(stack)
            ...
            .pApplicationName(stack.UTF8("lets-plot-graphite-offscreen-composite-probe"))
