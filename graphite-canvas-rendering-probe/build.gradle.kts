...
configurations.configureEach {
    resolutionStrategy.eachDependency {
        if (requested.group == "org.jetbrains.skiko") {
            ...
            because("Graphite Canvas probe requires a coherent Skiko 0.153 runtime")
        }
    }
}

application {
    mainClass.set("probe.GraphiteCanvasRenderingProbeKt")
}

tasks.register<JavaExec>("graphiteCanvasRenderingProbe") {
    group = "verification"
    description = "Compares the existing Lets-Plot SkiaContext2d on software and Vulkan Graphite surfaces."
