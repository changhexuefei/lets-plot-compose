...
configurations.configureEach {
    resolutionStrategy.eachDependency {
        if (requested.group == "org.jetbrains.skiko") {
            ...
            because("Graphite PlotCanvasDrawable probe requires a coherent Skiko 0.153 runtime")
        }
    }
}

application {
    mainClass.set("probe.GraphitePlotDrawableProbeKt")
}

tasks.register<JavaExec>("graphitePlotDrawableProbe") {
    group = "verification"
    description = "Paints one real Lets-Plot PlotCanvasDrawable to SOFTWARE and Vulkan Graphite surfaces."
