...
sourceSets {
    main {
        ...
        kotlin.include("org/jetbrains/letsPlot/compose/DesktopRenderPath.kt")
    }
}
...
configurations.configureEach {
    resolutionStrategy.eachDependency {
        if (requested.group == "org.jetbrains.skiko") {
            ...
            because("Graphite render-path provider probe requires a coherent Skiko 0.153 runtime")
        }
    }
}

application {
    mainClass.set("probe.GraphiteRenderPathProviderProbeKt")
}

tasks.register<JavaExec>("graphiteRenderPathProviderProbe") {
    group = "verification"
    description = "Binds the production render-path seam to a real Graphite offscreen provider."
