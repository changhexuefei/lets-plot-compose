...
fun main() {
    val outputDir = File(
        System.getenv("GRAPHITE_PROBE_OUTPUT_DIR")
            ?: "graphite-paint-bridge-probe/build/probe"
    ).apply { mkdirs() }
    val resultFile = outputDir.resolve("graphite-paint-bridge.txt")
    val softwarePng = outputDir.resolve("software-paint-bridge.png")
    val graphitePng = outputDir.resolve("graphite-paint-bridge.png")

    var stage = "startup"
    var prepared: PreparedPlot? = null
    val evidence = linkedMapOf(
        "schema" to "1",
        "probe.type" to "graphite-paint-bridge",
        "renderer.entry" to "paintOnSkiaCanvas -> PlotCanvasDrawable.paint",
        ...
        "plotpanel.integration" to "PAINT_BRIDGE_ONLY",
    ...
    try {
        ...
        check(difference <= MAX_PIXEL_DIFFERENCE_RATIO) {
            "Graphite paint bridge output diverged from SOFTWARE baseline: " +
                "difference=$difference max=$MAX_PIXEL_DIFFERENCE_RATIO"
        }
        evidence["comparison"] = "PASS"

        evidence["result"] = "PAINT_BRIDGE_CAPABLE"
        evidence["failure.stage"] = "none"
        writeEvidence(resultFile, evidence)
        println("GRAPHITE_PAINT_BRIDGE_RESULT PASS")
        ...
        writeEvidence(resultFile, evidence)
        println("GRAPHITE_PAINT_BRIDGE_RESULT FAIL stage=$stage type=" + evidence["failure.type"])
        throw t
    } finally {
...
private fun createVulkanObjects(): VulkanObjects {
    MemoryStack.stackPush().use { stack ->
        val appInfo = VkApplicationInfo.calloc(stack)
            ...
            .pApplicationName(stack.UTF8("lets-plot-graphite-paint-bridge-probe"))
