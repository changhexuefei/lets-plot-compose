...
fun main() {
    val outputDir = File(
        System.getenv("GRAPHITE_PROBE_OUTPUT_DIR")
            ?: "graphite-render-surface-probe/build/probe"
    ).apply { mkdirs() }
    val resultFile = outputDir.resolve("graphite-render-surface.txt")
    val pngFile = outputDir.resolve("graphite-render-surface.png")

    var stage = "startup"
    val evidence = linkedMapOf(
        "schema" to "1",
        "probe.type" to "graphite-render-surface",
        ...
        "vulkan.image.format" to "B8G8R8A8_UNORM",
    ...
    try {
        check(System.getProperty("os.name").startsWith("Windows", ignoreCase = true)) {
            "Render surface probe is intentionally Windows-only"
        }
        ...
        try {
            ...
            GraphiteContext.makeVulkan(
                ...
                context.makeRecorder().use { recorder ->
                    stage = "graphite-backend-texture"
                    BackendTexture.makeVulkan(
                        ...
                        surface.use {
                            ...
                            Paint().use { paint ->
                                paint.color = RED
                                surface.canvas.drawRect(16f, 16f, 48f, 48f, paint)
                            }
                            evidence["graphite.draw"] = "RECORDED"
                            ...
                            val pixels = readBackGraphiteImage(vk, target.image)
                            val center = pixels[(HEIGHT / 2) * WIDTH + (WIDTH / 2)]
                            val corner = pixels[4 * WIDTH + 4]
                            evidence["pixel.center.argb"] = "0x%08X".format(center)
                            evidence["pixel.corner.argb"] = "0x%08X".format(corner)
                            check(isRed(center)) { "Center pixel is not red: 0x%08X".format(center) }
                            check(isWhite(corner)) { "Corner pixel is not white: 0x%08X".format(corner) }

                            val buffered = BufferedImage(WIDTH, HEIGHT, BufferedImage.TYPE_INT_ARGB)
                            buffered.setRGB(0, 0, WIDTH, HEIGHT, pixels, 0, WIDTH)
                            check(ImageIO.write(buffered, "png", pngFile)) {
                                "Failed to encode Vulkan readback as PNG"
                            }

                            evidence["graphite.readback"] = "PASS"
                            evidence["graphite.readback.path"] = "vulkan-transfer-src"
                            evidence["graphite.png"] = pngFile.name
                        }
                    }
                }
            }
        ...
        evidence["result"] = "RENDER_SURFACE_CAPABLE"
        evidence["failure.stage"] = "none"
        writeEvidence(resultFile, evidence)
        println("GRAPHITE_RENDER_SURFACE_RESULT PASS")
        ...
        writeEvidence(resultFile, evidence)
        println("GRAPHITE_RENDER_SURFACE_RESULT FAIL stage=$stage type=" + evidence["failure.type"])
        throw t
    }
}
...
private fun createVulkanObjects(): VulkanObjects {
    MemoryStack.stackPush().use { stack ->
        val appInfo = VkApplicationInfo.calloc(stack)
            ...
            .pApplicationName(stack.UTF8("lets-plot-graphite-render-surface-probe"))
...
private fun isWhite(color: Int): Boolean {
    ...
    val g = (color ushr 8) and 0xFF
    val b = color and 0xFF
    return a >= 240 && r >= 220 && g >= 220 && b >= 220
}
